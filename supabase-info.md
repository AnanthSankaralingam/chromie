...
| `browserbase_context_id` | Identity-level Browserbase persisted Context (cookie jar) for `/new` custom recorded automations; frozen on first use. Corporate users inherit the earliest teammate's context for their work-email domain; consumer/free-email users get their own. eviivo/gov keep their own dedicated per-scenario contexts and do not use this. Applied via Supabase migration `add_profiles_browserbase_context_id`. |
...
**Recorder persisted login (identity-level context):** The `/new` recorder's Browserbase session attaches a persisted **Context** so third-party logins survive across recordings and scheduled runs. The context is **identity-level and frozen on `profiles.browserbase_context_id`** (not derived per-automation): a corporate user (work-email domain via `companyDomainFromEmail`) inherits the earliest teammate's context for that domain, while a consumer/free-email user gets their own. It is provisioned lazily and frozen on first use by `ensureProfileBrowserbaseContextId` (`src/lib/new-automation/recording-context.js`), which runs with the **service role** (corporate teammates live in other users' `profiles` rows that per-user RLS would hide). At recording start (`POST /api/new-automation-sessions`) the session runs with `browserSettings.context = { id, persist: true }` so the human's first login is saved back; on save the same id is stamped onto `automations.browserbase_context_id` so the runner (whose `resolve_browserbase_context_id` gives the per-automation column precedence) uses it directly and never falls back to the shared per-scenario `workflow_browser_contexts`. A brand-new context is an empty cookie jar — not authenticated until the user logs in once during a recording. eviivo/gov intentionally keep their **dedicated per-scenario** contexts (own reset/pinning lifecycle) and do not use the identity context.

**Recorder egress pinning (must match the runner):** Login-bound sites reject a restored context when the egress IP/fingerprint rotates, so the recording session pins region + Browserbase proxy + viewport via `resolveRecorderSessionPinning()` in `src/lib/browserbase.js` (`BROWSERBASE_RECORDER_*` env; proxies default ON, viewport `1920x1080`). The cookies captured during recording are minted under that egress, and the runner reuses the **same** identity via `resolve_session_pinning("recorder")` — the `BROWSERBASE_RECORDER_*` values must be mirrored in the runner's environment. The recording session is a human-driven login, so it passes `solveCaptchas: false` (like `scripts/eviivo_login.py`) so the real login challenge renders for the person to solve; the scheduled run later leaves the captcha solver at its default (on). On the runner, `custom_recorded_automation` also resolves the identity context from `profiles.browserbase_context_id` when the automation column is blank and never uses the per-scenario `workflow_browser_contexts` (which would leak one login jar across every tenant).

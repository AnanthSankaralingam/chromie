...
URL: {url}
Extension Intent: {intent_section}
Page HTML: {html_content}

Task: Analyze the HTML and identify 8-10 important UI elements.{intent_task}
...
Prioritize (or focus on intent if provided):
...
def scrape_with_hyperbrowser(url: str, profile_id: str | None = None) -> tuple[str, str]:
    """
    ...
    logger.info(f"Starting Hyperbrowser scrape for: {url}" + (f" (profile: {profile_id})" if profile_id else ""))
    
    session_kwargs = {"use_stealth": True, "accept_cookies": True}
    if profile_id:
        session_kwargs["profile"] = {"id": profile_id, "persistChanges": True}
    session_options = CreateSessionParams(**session_kwargs)
    ...
    # scrape_result.data is a ScrapeJobData object (Pydantic model), not a dict
    # Access attributes directly, not with .get()
    html_content = scrape_result.data.html or ""
    
    # Extract page title - try metadata first, then parse HTML
    page_title = None
    if scrape_result.data.metadata:
        page_title = scrape_result.data.metadata.get("title")
    
    if not page_title and html_content:
        soup = BeautifulSoup(html_content, "html.parser")
        page_title = soup.title.string if soup.title else None
    
    if not page_title:
        page_title = extract_domain_from_url(url)
    
    logger.info(f"✓ Hyperbrowser scrape complete. Page: '{page_title}' ({len(html_content):,} characters)")
    
    return page_title, html_content

def generate_summary_with_llm(page_title: str, url: str, html_content: str, intent: str | None = None) -> tuple[dict, dict]:
    """
    ...
    try:
        ...
        intent_section = f"Scraping Intent: {intent}\n\n" if intent else ""
        intent_task = (
            " Focus on elements most relevant to this intent."
            if intent
            else " that Chrome extensions commonly interact with."
        )
...
def scrape_url(url: str, intent: str | None = None, profile_id: str | None = None) -> dict:
    """
    ...
    try:
        page_title, html_content = scrape_with_hyperbrowser(url, profile_id=profile_id)
    ...
    summary, token_usage = generate_summary_with_llm(page_title, url, html_content, intent=intent)
    ...
    # Save to Supabase if successful (skip when intent provided - cache is for generic scrapes)
    if "error" not in summary.get("major_elements", {}):
        if not intent:
            ...
            logger.info("Skipping Supabase save (intent-specific scrape)")
...
def lambda_handler(event: Dict[str, Any], context: Any) -> Dict[str, Any]:
    ...
    Expected input format:
    {
        ...
        "intent": "Optional 1-2 sentence description of what to look for (e.g., element types or extension goal)",
        "profile_id": "Optional Hyperbrowser profile ID to use for the scrape session (e.g., logged-in state)"
    }
    
    Or for API Gateway:
    {
        "body": "{\"url\": \"https://example.com\", \"intent\": \"...\", \"profile_id\": \"...\"}"
    }
    ...
    try:
        ...
        if "body" in event:
            ...
            profile_id = event.get("profile_id") or None
        ...
        # Scrape the URL
        result = scrape_url(url, intent=intent, profile_id=profile_id)

...
export function useChat({ 
  ...
  useEffect(() => {
    ...
    // Reset local conversation state when project changes
    setPreviousResponseId(null)
    setConversationTokenTotal(0)
    console.log('[client/use-chat] reset conversation state')
  }, [projectId])

  // Public helper to clear conversation state when user starts fresh
  const startNewConversation = () => {
    setPreviousResponseId(null)
    setConversationTokenTotal(0)
    console.log('[client/use-chat] user started new conversation: cleared response id and token total')
  }
  ...
  const handleSendMessage = async (e) => {
    ...
    try {
      ...
      const data = await response.json()
      if (data?.errorType === 'context_window') {
        const msg = {
          role: 'assistant',
          content: 'Context limit reached. Please start a new conversation.'
        }
        setMessages(prev => [...prev, msg])
        const nextTotal = typeof data.nextConversationTokenTotal === 'number' ? data.nextConversationTokenTotal : conversationTokenTotal
        setConversationTokenTotal(nextTotal)
        console.log('[client/use-chat] context-window non-stream response handled', { nextTotal })
        return
      }
      if (data?.errorType === 'context_window') {
        const msg = {
      ...
      if (content && !data.requiresUrl) {
        ...
        if (data?.nextResponseId) {
          setPreviousResponseId(data.nextResponseId)
          console.log('[client/use-chat] stored nextResponseId', data.nextResponseId)
        }
        if (typeof data?.tokensUsedThisRequest === 'number') {
          const nextTotal = (conversationTokenTotal || 0) + data.tokensUsedThisRequest
          setConversationTokenTotal(nextTotal)
          console.log('[client/use-chat] updated conversationTokenTotal', { tokensUsedThisRequest: data.tokensUsedThisRequest, nextTotal })
        }

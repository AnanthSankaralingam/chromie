...
export async function POST(request, { params }) {
  ...
  try {
    console.log("🤖 Generating HyperAgent test script for project:", projectId)
    ...
    console.log("✅ HyperAgent test script generated successfully")
    ...
    if (saveError) {
      console.error("Error saving HyperAgent test script:", saveError)
      return NextResponse.json({ error: "Failed to save test script" }, { status: 500 })
    }

    return NextResponse.json({
      success: true,
      message: "HyperAgent test script generated successfully",
      script: testScript,
    })

  } catch (error) {
    console.error("❌ HyperAgent test script generation failed:", error)
    
    return NextResponse.json({
      success: false,
      error: error.message || "HyperAgent test script generation failed",

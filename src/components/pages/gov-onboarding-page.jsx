        ...
        <div className="flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-3 max-w-2xl text-sm leading-6 text-zinc-400">
              Chromie uses this profile to configure SAM.gov searches, rank opportunities, and
              connect teammates from the same organization.
            </p>
          </div>
        </div>
          ...
          <Card className={`mt-8 ${CARD_CLASS}`}>
            <CardContent className="px-5 py-6">
              <p className="text-sm text-zinc-400">
                Sign in or create an account to start government onboarding.
              </p>
              <Button className={`mt-5 ${BTN_PRIMARY}`} onClick={() => setShowAuth(true)}>
                Sign in to continue
              </Button>
            </CardContent>
          </Card>
            ...
            <Card className={`mt-8 ${CARD_CLASS}`}>
              <CardHeader className="border-b border-white/10 pb-4">
                ...
                <CardDescription className="text-zinc-400">
                  Your signed-in email is {user.email}. We use the company domain to link teammates
                  to the same shared profile.
                </CardDescription>
              </CardHeader>
              <CardContent className="space-y-5 pt-6">
                {likelyPersonalEmail ? (
                  <div className="border border-amber-400/25 bg-amber-400/10 px-3 py-2 text-sm text-amber-100">
                ...
                {error ? (
                  <div className="border border-red-500/25 bg-red-500/10 px-3 py-2 text-sm text-red-300">
                    {error}
                  </div>
                ) : null}
                ...
                <Field label="Company overview" hint="Short context used for future fit analysis.">
                  <textarea
                    rows={5}
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  <Field label="Search keywords" hint="One per line.">
                    <textarea
                      rows={5}

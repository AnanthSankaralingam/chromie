...
export default function StreamingChat({
  ...
  const chatState = useChatState(projectId, hasGeneratedCodeProp, isAdminMode, autoGeneratePrompt)
  const {
    inputMessage,
    setInputMessage,
    messages,
  ...
  // Auto-generation effect — add user message if not already present, then start generation
  // Message may already be in messages from useChatState's loadMessages (when autoGeneratePrompt + empty DB)
  useEffect(() => {
    if (
      isAdminMode ||
      !autoGeneratePrompt ||
    ...
    setMessages((prev) => {
      const hasUserMessage = prev.some(m => m.role === 'user' && m.content === autoGeneratePrompt)
      if (hasUserMessage) return prev
      return [...prev, { role: "user", content: autoGeneratePrompt }]
    })
    startGeneration(autoGeneratePrompt, true)
  }, [isAdminMode, autoGeneratePrompt, isProjectReady, messagesLoaded, hasGeneratedCode, isGenerating])
  ...
  const handleFrontendTypeCancel = () => {
    chatState.setIsGenerating(false)
    currentRequestRef.current = null
  }
        ...
        >
          <div className="relative">
            <AIInputWithSearch
              ...
              disabled={isGenerating || !projectId || isAdminMode}
              className="py-0"

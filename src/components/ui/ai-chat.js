...
export default function AIChat({ projectId, autoGeneratePrompt, onAutoGenerateComplete, onCodeGenerated, onGenerationStart, onGenerationEnd, isProjectReady }) {
  ...
  useEffect(() => {
    ...
    } else if (autoGeneratePrompt && autoGeneratedPrompts.current.has(autoGeneratePrompt)) {
      console.log('Auto-generate prompt already processed:', autoGeneratePrompt)
    }
  }, [autoGeneratePrompt, isProjectReady, isGenerating])

  // Persist autoGeneratePrompt until project is ready
  useEffect(() => {
    if (autoGeneratePrompt && !isProjectReady) {
      console.log('Waiting for project to be ready before auto-generating:', autoGeneratePrompt)
    }
  }, [autoGeneratePrompt, isProjectReady])

  // Retry auto-generation when project becomes ready
  useEffect(() => {
    if (autoGeneratePrompt && isProjectReady && !isGenerating && autoGeneratedPrompts.current.has(autoGeneratePrompt)) {
      console.log('Project is now ready, retrying auto-generation for:', autoGeneratePrompt)
      // Remove from processed set to allow retry
      autoGeneratedPrompts.current.delete(autoGeneratePrompt)
    }
  }, [isProjectReady, autoGeneratePrompt, isGenerating])

  // Monitor project readiness and trigger auto-generation if there's a pending prompt
  useEffect(() => {
    if (autoGeneratePrompt && isProjectReady && !isGenerating && !autoGeneratedPrompts.current.has(autoGeneratePrompt)) {
      console.log('Project became ready, triggering auto-generation for pending prompt:', autoGeneratePrompt)
      // This will trigger the main auto-generation effect
    }
  }, [isProjectReady, autoGeneratePrompt, isGenerating])
